...
/* Internal extractor resolving base vectors from HoA or AoH data references */
static double* extract_vector(SV* data_sv, const char* var_name, int power, unsigned int *out_n) {
    unsigned int n = 0;
    double *restrict vec = NULL;

    if (SvTYPE(SvRV(data_sv)) == SVt_PVHV) {
        /* Hash of Arrays (HoA) mode */
        HV* hv = (HV*)SvRV(data_sv);
        SV** col_sv = hv_fetch(hv, var_name, strlen(var_name), 0);
        if (!col_sv || !SvROK(*col_sv) || SvTYPE(SvRV(*col_sv)) != SVt_PVAV)
            croak("lm: Column '%s' not found or not an array reference in data", var_name);
        ...
        *out_n = n;
        ...
        for (unsigned int i = 0; i < n; i++) {
            ...
            double v = (val && SvOK(*val)) ? SvNV(*val) : 0.0;
            vec[i] = power == 1 ? v : pow(v, power);
        }
    } else if (SvTYPE(SvRV(data_sv)) == SVt_PVAV) {
        /* Array of Hashes (AoH) mode */
        AV* av = (AV*)SvRV(data_sv);
        n = av_len(av) + 1;
        *out_n = n;
        Newx(vec, n, double);
        for (unsigned int i = 0; i < n; i++) {
            ...
            if (!row_sv || !SvROK(*row_sv) || SvTYPE(SvRV(*row_sv)) != SVt_PVHV)
                croak("lm: Data array must contain hash references");
            
            HV* row_hv = (HV*)SvRV(*row_sv);
            SV** val = hv_fetch(row_hv, var_name, strlen(var_name), 0);
            double v = (val && SvOK(*val)) ? SvNV(*val) : 0.0;
            vec[i] = power == 1 ? v : pow(v, power);
        }
    } else {
        croak("lm: Data must be a Hash of Arrays or an Array of Hashes");
    }
    return vec;
}

/* Recursive formula resolver: dynamically handles operations like "wt:hp" or "I(hp^2)" */
static double* fetch_term_vector(SV* data_sv, const char* term, unsigned int *out_n) {
    char term_cpy[256];
    strncpy(term_cpy, term, 255); term_cpy[255] = '\0';

    /* Parse explicit interactions (e.g., wt:hp) */
    ...
    if (colon) {
        *colon = '\0';
        ...
        double *v1 = fetch_term_vector(data_sv, left, &n1);
        double *v2 = fetch_term_vector(data_sv, right, &n2);

        if (n1 != n2 || n1 == 0) croak("lm: interaction size mismatch");
        *out_n = n1;

        double *vec;
        Newx(vec, n1, double);
        for (unsigned int i = 0; i < n1; i++) vec[i] = v1[i] * v2[i];

        Safefree(v1); Safefree(v2);
        return vec;
    }

    /* Parse mathematical powers wrapped in R's AsIs function: I(hp^2) */
    if (strncmp(term_cpy, "I(", 2) == 0) {
        char *end = strrchr(term_cpy, ')');
        if (end) *end = '\0';
        char *inner = term_cpy + 2; /* skip "I(" */
        char *caret = strchr(inner, '^');
        if (caret) {
            *caret = '\0';
            int power = atoi(caret + 1);
            return extract_vector(data_sv, inner, power, out_n);
        }
        return extract_vector(data_sv, inner, 1, out_n);
    }

    /* Standard variable fallback */
    return extract_vector(data_sv, term_cpy, 1, out_n);
}
    ...
    CODE:
    {
        ...
        *tilde = '\0';
        ...
        while (chunk != NULL) {
            ...
            /* Check for interaction macro '*', expand 'A*B' into 'A', 'B', 'A:B' */
            ...
            if (star) {
                *star = '\0';
                ...
                /* Handle explicit ':' or standard terms */
                /* Strip bare degree operators here too */
        ...
        /* Deduplicate terms (so A*B + A resolves cleanly without rank deficient matrix) */
        ...
        /* --- 3. Build Matrices (X, Y) --- */
        double *restrict Y = fetch_term_vector(data_sv, lhs, &n);
        double *restrict X;
        Newx(X, n * p, double);
        for (unsigned int j = 0; j < p; j++) {
            if (strcmp(uniq_terms[j], "Intercept") == 0) {
                for (unsigned int i = 0; i < n; i++) X[i * p + j] = 1.0;
            } else {
                unsigned int curr_n = 0;
                double *v = fetch_term_vector(data_sv, uniq_terms[j], &curr_n);
                if (curr_n != n) croak("lm: mismatched row count on %s", uniq_terms[j]);
                for (unsigned int i = 0; i < n; i++) X[i * p + j] = v[i];
                Safefree(v);
            }
        }
        ...
        AV *restrict fitted_av = newAV();
        AV *restrict resid_av = newAV();
        av_extend(fitted_av, n - 1);
        av_extend(resid_av, n - 1);
        ...
        for (unsigned int i = 0; i < n; i++) {
            ...
            av_store(fitted_av, i, newSVnv(y_hat));
            av_store(resid_av, i, newSVnv(res));
        }

        hv_store(res_hv, "coefficients",  12, newRV_noinc((SV*)coef_hv), 0);
        hv_store(res_hv, "fitted.values", 13, newRV_noinc((SV*)fitted_av), 0);
        hv_store(res_hv, "residuals",     9,  newRV_noinc((SV*)resid_av), 0);
