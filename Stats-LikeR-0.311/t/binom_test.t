...
#	two.sided
{
	...
	is_approx($r->{'null.value'}, 0.5,             'C1 null value');
	is_approx($r->{'p.value'},    0.503444671631,  'C1 two.sided p-value');
	is_approx($r->{'conf.int'}[0], 0.191190060725, 'C1 CI lower');
	is_approx($r->{'conf.int'}[1], 0.639457412693, 'C1 CI upper');
}

# Exact rational p-value: 2*(C(10,0)+C(10,1)+C(10,2))/2^10 = 112/1024
{
	my $r = binom_test(2, 10);
	is_approx($r->{'p.value'},     0.109375,        'C2 two.sided p-value (exact)');
	is_approx($r->{'conf.int'}[0], 0.0252107263268, 'C2 CI lower');
	is_approx($r->{'conf.int'}[1], 0.556095462308,  'C2 CI upper');
}

# Boundary x == 0
{
	my $r = binom_test(0, 10);
	is_approx($r->{'p.value'},     0.001953125,     'C3 two.sided p-value (x=0)');
	is_approx($r->{'conf.int'}[0], 0,               'C3 CI lower is 0');
	is_approx($r->{'conf.int'}[1], 0.308497107819,  'C3 CI upper');
}

# Boundary x == n, with p != 0.5  (p-value = 0.3^10)
{
	my $r = binom_test(10, 10, p => 0.3);
	is_approx($r->{'p.value'},     5.9049e-06,      'C4 two.sided p-value (x=n)', 1e-6);
	is_approx($r->{'conf.int'}[0], 0.691502892181,  'C4 CI lower');
	is_approx($r->{'conf.int'}[1], 1,               'C4 CI upper is 1');
}

# Larger n, non-default p
{
	my $r = binom_test(683, 1000, p => 0.7);
	is_approx($r->{'p.value'},     0.240871445717,  'C5 two.sided p-value (n=1000)');
	is_approx($r->{'conf.int'}[0], 0.653152969441,  'C5 CI lower');
	is_approx($r->{'conf.int'}[1], 0.711765143274,  'C5 CI upper');
}
...
#	one-sided
{
	...
	is_approx($r->{'p.value'},     0.0701908264,    'C6 greater p-value');
	is_approx($r->{'conf.int'}[0], 0.0872644339142, 'C6 greater CI lower');
	is_approx($r->{'conf.int'}[1], 1,               'C6 greater CI upper is 1');
}
{
	my $r = binom_test(15, 20, alternative => 'less');
	is_approx($r->{'p.value'},     0.994091033936,  'C7 less p-value');
	is_approx($r->{'conf.int'}[0], 0,               'C7 less CI lower is 0');
	is_approx($r->{'conf.int'}[1], 0.89591916409,   'C7 less CI upper');
}
{
	my $r = binom_test(8, 20, alternative => 'greater');
	is_approx($r->{'p.value'},     0.868412017822,  'C8 greater p-value');
	is_approx($r->{'conf.int'}[0], 0.21706858937,   'C8 greater CI lower');
}
...
#	array-ref form: x = [successes, failures]
{
	...
	is_approx($r->{'p.value'}, 0.503444671631,       'C9 p-value matches scalar form');
}
...
#	conf_level / conf.level spellings agree
{
	...
	is_approx($x->{'conf.int'}[0], $y->{'conf.int'}[0], 'C10 conf_level == conf.level (lo)');
	is_approx($x->{'conf.int'}[1], $y->{'conf.int'}[1], 'C10 conf_level == conf.level (hi)');
	cmp_ok($x->{'conf.int'}[1] - $x->{'conf.int'}[0], '>',
