...
# --- basic scalar mean ---------------------------------------------------
my $s;
no_leaks_ok { eval { $s = mean(1, 2, 3, 4) } } 'mean no leaks: scalars';
is_approx $s, 2.5, 'mean of a flat scalar list';

# --- array-ref mean ------------------------------------------------------
my $a;
no_leaks_ok { eval { $a = mean([1, 2, 3, 4]) } } 'mean no leaks: array ref';
is_approx $a, 2.5, 'mean of a single array ref';

# --- mixed scalars and array refs ----------------------------------------
my $m;
no_leaks_ok { eval { $m = mean(1, [2, 3], 4) } } 'mean no leaks: mixed';
is_approx $m, 2.5, 'mean across scalars and array refs together';

# --- single element ------------------------------------------------------
my $one;
no_leaks_ok { eval { $one = mean(42) } } 'mean no leaks: single element';
is_approx $one, 42, 'mean of one element is that element';

# --- negatives -----------------------------------------------------------
my $neg;
no_leaks_ok { eval { $neg = mean(-2, 0, 2) } } 'mean no leaks: negatives';
is_approx $neg, 0, 'mean handles negative values';

# --- croak on empty input ------------------------------------------------
my $e_empty = '';
no_leaks_ok { eval { mean(); 1 } or $e_empty = $@ } 'mean no leaks: empty croak';
...
my $e_scalar = '';
no_leaks_ok { eval { mean(1, undef, 3); 1 } or $e_scalar = $@ } 'mean no leaks: undef scalar croak';
...
my $e_aref = '';
no_leaks_ok { eval { mean([1, undef, 3]); 1 } or $e_aref = $@ } 'mean no leaks: undef in aref croak';
