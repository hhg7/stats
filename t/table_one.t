...
is_approx($age->{'p.value'}, t_test(\@ageA,\@ageB)->{'p.value'}, 'age p-value matches t_test');
...
# approximation exactly as R does; table_one keeps that warning to itself
{
	...
	is_approx($sex_hdr->{'p.value'}, chisq_test([[2,4],[3,2]])->{'p.value'}, 'sex p-value matches chisq_test');
}
...
is_approx($par->[0]{'p.value'}, aov(\%hoa,'val ~ arm')->{arm}{'Pr(>F)'}, 'ANOVA p matches aov');
is($np->[0]{test}, 'kruskal-wallis', '3 groups nonparametric => Kruskal-Wallis');
is_approx($np->[0]{'p.value'},
	kruskal_test([5.1,4.9,6.2,6.1,6.9,7.2,8,9,10],[qw(A A A B B B C C C)])->{'p.value'},
...
ok(!defined row_for($overall,'age')->{'p.value'}, 'no-by: no p-value');
