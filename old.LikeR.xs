 ...
 * ----------------------------------------------------------------------- */
static double kendall_exact_pvalue(unsigned int n, double s_obs, const char *restrict alt) {
...
static void sweep_matrix_aov(double** m, int p, int k) {
    double pivot = m[k][k];
    int i, j;
    if (fabs(pivot) < 1e-12) return;
    for (j = 0; j < p; j++) if (j != k) m[k][j] /= pivot;
    for (i = 0; i < p; i++) {
        if (i == k) continue;
        double factor = m[i][k];
...
double *restrict X; Newx(X, n * p, double);
        ...
        Safefree(row_names); /* We now use valid_row_names */
        ...
        bool *restrict aliased;
        Newx(aliased, p, bool);
        /* Replaces the fragile invert_matrix_gj with Sweep */
        int final_rank = sweep_matrix_ols(XtX, p, aliased);

        double *restrict beta; Newxz(beta, p, double);
        for (unsigned int i = 0; i < p; i++) {
            if (aliased[i]) {
                beta[i] = NAN; /* Match R's aliased variables */
            } else {
                double sum = 0.0;
                for (unsigned int j = 0; j < p; j++) {
                    if (!aliased[j]) sum += XtX[i * p + j] * XtY[j];
                }
                beta[i] = sum;
            }
        }
        ...
        /* Calculate Fitted Values, Residuals, and RSS */
        ...
        // Calculate Summary Statistics (Std Error, t-value, p-value)
        HV *restrict summary_hv = newHV();
        AV *restrict terms_av = newAV();
        int df_res = valid_n - final_rank;
        double rse_sq = (df_res > 0) ? (rss / df_res) : NAN;

        for (unsigned int j = 0; j < p; j++) {
            // 1. Add to flat coefficients hash
            hv_store(coef_hv, uniq_terms[j], strlen(uniq_terms[j]), newSVnv(beta[j]), 0);
            
            // 2. Record term order for Perl formatting
            av_push(terms_av, newSVpv(uniq_terms[j], 0));
            
            // 3. Build summary row HashRef/
            ...
            if (aliased[j]) {
                ...
                /* Sweep replaced XtX with its inverse. XtX[j * p + j] is the diagonal element */
        ...
        /* Store everything in the main return hash */
...
aov(data_ref, formula_sv)
    ...
    {
        HV* data;
        char *f_str, *lhs, *rhs, *token, *saveptr;
        char **terms;
        int num_terms = 0, i, j, row;
        size_t n;
        double **xtx, rss_prev, rss_curr;
        HV* ret_hash;
        ...
        if (!lhs || !rhs) { Safefree(f_str); croak("aov: invalid formula"); }
        
        // Trim whitespace from response
        while(isspace(*lhs)) lhs++;
        token = lhs + strlen(lhs) - 1;
        while(token > lhs && isspace(*token)) *token-- = '\0';

        // Get row count from first available array
        HE* entry;
        hv_iterinit(data);
        if (!(entry = hv_iternext(data))) { Safefree(f_str); croak("aov: empty data"); }
        n = av_len((AV*)SvRV(hv_iterval(data, entry))) + 1;
        ...
        // Build Augmented Cross-product Matrix (p+1 x p+1)
        int p = num_terms + 1; // +1 for Intercept
        xtx = (double**)safemalloc((p + 1) * sizeof(double*));
        for (i = 0; i <= p; i++) {
            xtx[i] = (double*)safemalloc((p + 1) * sizeof(double));
            for (j = 0; j <= p; j++) xtx[i][j] = 0.0;
        }
        ...
        for (row = 0; row < n; row++) {
            ...
            for (i = 0; i < num_terms; i++) row_vec[i+1] = evaluate_term_aov(data, terms[i], row);
        ...
        // Sequential SS via Sweep
        sweep_matrix_aov(xtx, p + 1, 0); // Intercept
        rss_prev = xtx[p][p];

        for (i = 0; i < num_terms; i++) {
            sweep_matrix_aov(xtx, p + 1, i + 1);
            rss_curr = xtx[p][p];
            double ss_term = rss_prev - rss_curr;
            rss_prev = rss_curr;
            ...
            // FIXED: Using hv_store with strlen for dynamic variable names
            hv_store(ret_hash, terms[i], strlen(terms[i]), newRV_noinc((SV*)term_stats), 0);
        }
        ...
        for (i = 0; i < num_terms; i++) {
            SV** term_sv = hv_fetch(ret_hash, terms[i], strlen(terms[i]), 0);
            HV* t_hv = (HV*)SvRV(*term_sv);
            ...
            if (ms_res > 0) {
                ...
                hv_stores(t_hv, "Pr(>F)", newSVnv(1.0 - pf(f_val, 1.0, res_df)));
            } else {
                // Supply NAN so that the Perl hash keys always exist for the user
